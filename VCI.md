...
- API-key entry flow (single `sk-` input, stored in `localStorage` for
  personal use; ephemeral-token backend for anything public — see § 11).
...
> **MCP is optional.** A perfectly good VCI app has zero MCP servers.
> Only add MCP when you need to reach outside the app's own state
> (e.g. sync to an external service).
...
### 10.2 Remote vs local MCP servers
...
Local (stdio) MCP servers cannot be used directly from a browser. To
integrate them, you need a small backend proxy that connects to the
local server and exposes an SSE endpoint — at which point they are
functionally remote MCP servers again.

Finding servers:
- Public directories: `mcpservers.org`, `github.com/modelcontextprotocol/servers`.
- First-party endpoints from Notion, Linear, GitHub, Slack, Sanity, Vercel, etc.
- Roll your own: a minimal MCP server is ~150 lines of code.

### 10.3 Wiring MCP tools
...
2. In the `ui`, add a small settings panel where the user can
   enable/disable MCP servers and paste tokens.
...
### 10.4 MCP security & approvals

**MCP is a real security surface.** Every MCP server you add can, in
principle, call arbitrary tools with arbitrary arguments generated by
the model. Treat MCP servers like npm packages: audit before use.
...
| `"never"`   | MCP tools auto-execute without user confirmation.                    | Trusted first-party read-only servers you control.                  |

Token handling:
- **Personal-use apps:** store MCP auth tokens in `localStorage`, same
  trade-off as the OpenAI key.
- **Public apps:** mint short-lived MCP tokens on your backend.
- **Whitelist tools** per server via `allowed_tools` whenever possible.

### 10.5 When to add MCP

| Add MCP when…                                                                       | Skip MCP when…                                                        |
|-------------------------------------------------------------------------------------|-----------------------------------------------------------------------|
| You need to reach an external service you don't control.                            | The app is entirely local — state stays in `localStorage`.            |
...
- **For MCP:** mint MCP tokens on the backend, too. Never ship
  long-lived OAuth tokens to the browser. See § 10.4.
...
## 13. Implementation checklist for an AI coder
...
- [ ] Decide whether to include MCP support (§ 10). If yes, follow
      § 10.3 for wiring.
